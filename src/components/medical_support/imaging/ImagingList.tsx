...
export default function ImagingList() {
  ...
  const [selectedItem, setSelectedItem] = React.useState<ImagingExam | null>(null);
    ...
    return [
      {
        ...
        fields: [
          {
            label: "영상검사아이디",
            value: safeValue(selectedItem.imagingExamId),
          },
          {
            label: "검사수행아이디",
      ...
      },
      {
        ...
        fields: [
          {
            label: "영상검사유형",
            value: safeValue(selectedItem.imagingType),
          },
        ],
      },
      {
  ...
  const handleOpenDetail = (item: ImagingExam) => {
    ...
    <MainLayout showSidebar={false}>
      <Stack spacing={2}>
        ...
        <ExamDetailDialog
          ...
          editHref={`/medical_support/imaging/edit/${selectedItem?.imagingExamId ?? ""}`}
