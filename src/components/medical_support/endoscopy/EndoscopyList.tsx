...
import type { EndoscopyExam } from "@/features/medical_support/endoscopy/endoscopyType";
...
export default function EndoscopyList() {
  ...
  const selected = React.useMemo(
    () =>
      items.find((item) => String(item.endoscopyExamId) === String(selectedId)) ??
      null,
    [items, selectedId]
  );
                    ...
                    >
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell align="center">내시경검사아이디</TableCell>
                          ...
                          <TableCell align="center">시술자아이디</TableCell>
                      ...
                      <TableBody>
                        ...
                        {paginatedItems.map((item, index) => (
                          ...
                          >
                            ...
                            <TableCell align="center">{safeValue(item.endoscopyExamId)}</TableCell>
                            <TableCell align="center">{safeValue(item.testExecutionId)}</TableCell>
                            <TableCell align="center">{safeValue(item.procedureRoom)}</TableCell>
                            <TableCell align="center">{safeValue(item.equipment)}</TableCell>
                            <TableCell align="center">{formatYn(item.sedationYn)}</TableCell>
                            <TableCell align="center">{safeValue(item.operationId)}</TableCell>
                            <TableCell align="center">{formatDateTime(item.procedureAt)}</TableCell>
                            <TableCell align="center">
                              <Chip
            ...
            <Card sx={{ borderRadius: 3, border: "1px solid var(--line)" }}>
              <CardContent sx={{ p: 2.5 }}>
                ...
                <Box sx={{ mt: 2, p: 2, borderRadius: 2, bgcolor: "rgba(255,255,255,0.7)" }}>
                  <Row label="내시경검사아이디" value={safeValue(activeSelected?.endoscopyExamId)} />
                  <Row label="검사수행아이디" value={safeValue(activeSelected?.testExecutionId)} />
                  <Row label="시술실" value={safeValue(activeSelected?.procedureRoom)} />
                  <Row label="장비" value={safeValue(activeSelected?.equipment)} />
                  <Row label="진정여부" value={formatYn(activeSelected?.sedationYn)} />
                  <Row label="시술자아이디" value={safeValue(activeSelected?.operationId)} />
                  <Row label="시술일시" value={formatDateTime(activeSelected?.procedureAt)} />
